#!/usr/bin/env bash
# Erudi Backend Setup Script - Mac Intel
...
if [ "$current_dir" == "backend" ]; then
    ...
    req_dev="./requirements/entrypoints/dev/mac-intel.txt"
    req_prod="./requirements/entrypoints/prod/mac-intel-prod.txt"
    venv_path="./venv"
else
    write_status "Running from erudi/ (don't forget to cd backend/ after setup)"
    req_dev="./backend/requirements/entrypoints/dev/mac-intel.txt"
    req_prod="./backend/requirements/entrypoints/prod/mac-intel-prod.txt"
