#!/usr/bin/env bash
# Erudi Backend Setup Script - Mac Silicon (M1/M2/M3+)
...
if [ "$current_dir" == "backend" ]; then
    ...
    req_dev="./requirements/entrypoints/dev/mac-silicon.txt"
    req_prod="./requirements/entrypoints/prod/mac-silicon-prod.txt"
    venv_path="./venv"
else
    write_status "Running from erudi/ (don't forget to cd backend/ after setup)"
    req_dev="./backend/requirements/entrypoints/dev/mac-silicon.txt"
    req_prod="./backend/requirements/entrypoints/prod/mac-silicon-prod.txt"
...
if [[ $version =~ ([0-9]+)\.([0-9]+)\.([0-9]+) ]]; then
    ...
    if (( major != 3 || minor != 12 )); then
        write_error "Python 3.12 exactly is required (pgserver publishes cp312 wheels only), found: $version"
