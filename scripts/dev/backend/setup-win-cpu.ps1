...
if ($currentDir -eq "backend") {
    ...
    $reqDev = ".\requirements\entrypoints\dev\win-cpu.txt"
    $reqProd = ".\requirements\entrypoints\prod\win-cpu-prod.txt"
    $venvPath = ".\venv"
} else {
    Write-Status "Running from erudi\ (don't forget to cd backend\ after setup)"
    $reqDev = ".\backend\requirements\entrypoints\dev\win-cpu.txt"
    $reqProd = ".\backend\requirements\entrypoints\prod\win-cpu-prod.txt"
