...
| ~~**Windows = CUDA-only.** La release installe `win-cuda-121-prod.txt` + `backend.spec` (CUDA) ; aucun `llama-server` CPU n'est bundlé → sur une machine Windows **sans NVIDIA**, le fallback CPU n'a pas de binaire à lancer. (À confirmer au runtime.)~~ **RÉSOLU** (#103, #77, juin 2026) : la release a cinq legs, Windows et Linux publient chacun une build CPU et une build CUDA sur deux canaux de mise à jour. Le fichier de requirements cité n'existe plus. | Élevée (produit) | `release.yml:43,47`, `backend.spec:53-65` (cuda/bin only, `if IS_WIN`) |
