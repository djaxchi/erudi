...
**A path we found and closed.** An audit of this page's own claims in September 2026 turned up a request that could have happened and is not in the table above, because nothing in the app triggered it — your own computer would have. LangChain, which runs the agent, ships with a tracing client called LangSmith. That client is switched on by an environment variable, and it uploads the entire exchange: the system prompt, the knowledge-base excerpts, your question and the model's answer. It arrived inside the build as a dependency of LangChain, the app inherited your whole environment when it started the backend, and nothing turned it off. Anyone who had once exported `LANGCHAIN_TRACING_V2` for an unrelated project would have had their Erudi conversations sent to `api.smith.langchain.com`, silently.

Nobody's data is known to have gone anywhere, and it was never on by default. But it was reachable, and this page claimed to list every request. It is now shut in two places: the launcher forces both tracing switches off on every start rather than merely defaulting them, and the Electron process strips the whole `LANGCHAIN_*` / `LANGSMITH_*` family from the environment before the backend ever sees it. Both are covered by tests. The same audit found that the Hugging Face telemetry switch had the same weakness — it was a default rather than an assignment, so an inherited `HF_HUB_DISABLE_TELEMETRY=0` re-enabled it — and that is now an assignment too.
...
- `ERUDI_ALLOW_LANGSMITH_TRACING=1` — the deliberate escape hatch for the path described above, for a contributor debugging the agent layer. It is the only way to make conversations leave your machine through LangChain, it has to be typed out in full, and it should never be set in a build anyone distributes.
...
The one thing this search did surface is the LangSmith path described above, which is why that section exists.
