...
def process_pdfs_to_causal_dataset(
    input_folders, chunk_size=800, overlap=200, output_path="data/training_datasets/"
):

    logging.info(
        f"Processing PDFs and TXT files to create a dataset, from folders: {input_folders}"
    )
    start = datetime.now()
    all_chunks = []
    pdf_files = [
        list(Path(input_folder).glob("*.pdf")) for input_folder in input_folders
    ]
    pdf_files = [item for sublist in pdf_files for item in sublist]

    txt_files = [
        list(Path(input_folder).glob("*.txt")) for input_folder in input_folders
    ]
    ...
    logging.info(
        f"Dataset created with {len(all_chunks)} chunks in {output_path}. in {datetime.now() - start} seconds"
    )
