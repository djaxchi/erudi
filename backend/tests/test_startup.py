...
class TestStartup_Endpoints:
    ...
    def test_get_welcome_popup_status_first_time(
    ...
    def test_get_welcome_popup_status_subsequent_calls(
        self, client: TestClient, test_db_session: Session
    ):
    ...
    def test_get_welcome_popup_status_existing_record_true(
        self, client: TestClient, test_db_session: Session
    ):
    ...
    def test_get_welcome_popup_status_existing_record_false(
        self, client: TestClient, test_db_session: Session
    ):
...
class TestStartup_Schemas:
    ...
    def test_welcome_popup_response_json_serialization(self):
        ...
        assert (
            '"has_already_displayed":false' in json_str
            or '"has_already_displayed": false' in json_str
        )
...
class TestStartup_Integration:
    ...
    def test_concurrent_first_time_calls(self, client: TestClient, test_db_session: Session):
        ...
        results = [
            response1.json()["has_already_displayed"],
            response2.json()["has_already_displayed"],
        ]
