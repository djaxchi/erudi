...
class Embedder_Engine:
    ...
    Usage Pattern:
    1. Call get_embedder() to obtain embedder instance (loads if needed)
    2. Use embedder for encoding text (KB creation, query embedding, caching)
    3. Call cleanup() after batch operations to free memory
    ...
    See Also:
        ...
        cache_conversation_turn: Uses embedder for conversation caching
    """
    _instance = None
    ...
    def __init__(self):
        ...
        Examples:
            ...
            >>> embedder = Embedder_Engine.get_embedder()
        """
        raise RuntimeError("Use get_embedder() instead of instantiating Embedder_Engine directly")
    ...
    def get_embedder(cls):
        ...
        from sentence_transformers import SentenceTransformer
        if cls._instance is None:
            logger.info("Loading Embedder Engine (paraphrase-multilingual-MiniLM-L12-v2)")
            os.makedirs(CACHE_DIR, exist_ok=True)
            cls._instance = SentenceTransformer(
                "sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2",
                cache_folder=CACHE_DIR,
            )
            logger.info("Embedder Engine loaded successfully")
        return cls._instance
    ...
    def cleanup(cls) -> None:
        ...
        Examples:
            ...
            >>> # Cleanup after batch
            >>> Embedder_Engine.cleanup()
            >>> # INFO: Embedder Engine cleaned up
            >>> 
            >>> # Model will reload on next get_embedder()
            >>> embedder = Embedder_Engine.get_embedder()
            >>> # INFO: Loading Embedder Engine...
        ...
        if cls._instance is not None:
            del cls._instance
            cls._instance = None
            logger.info("Embedder Engine cleaned up")
