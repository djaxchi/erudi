  ...
  return new Promise((resolve, reject) => {
    ...
    killExistingBackend().then(() => {
      ...
      backendProcess.stderr.on('data', (data) => {
        ...
        const detectError = (line) => {
          ...
          if (line.includes('pynvml') || line.includes('NVML')) {
            return { code: 'NVIDIA_ML_ERROR', message: 'NVIDIA GPU runtime unavailable; falling back to CPU' };
          }
          
          // MLX detection errors (Mac Silicon specific)
          if (line.includes('mlx.core') || line.includes('MLX')) {
            return { code: 'MLX_ERROR', message: 'MLX framework error; verify Mac Silicon support' };
          }
          
          // PyTorch errors (works on all builds)
          if (line.includes('torch.cuda') || line.includes('cuda:') || line.includes('CUDA')) {
            return { code: 'TORCH_CUDA_ERROR', message: 'PyTorch CUDA unavailable; check GPU drivers' };
          }
          if (line.includes('No module named torch')) {
            return { code: 'PYTORCH_MISSING', message: 'PyTorch not installed in backend environment' };
          }
      ...
      backendProcess.on('exit', (code, signal) => {
        ...
        if (code !== 0 && code !== null) {
          if (code === 1) {
            errorEvent = { code: 'BACKEND_STARTUP_FAILED', message: `Backend exited with code ${code}` };
          } else if (code === 127) {
            errorEvent = { code: 'BACKEND_NOT_FOUND', message: 'Backend executable not found' };
          } else {
            errorEvent = { code: 'BACKEND_EXIT_ERROR', message: `Backend exited unexpectedly (code: ${code})` };
          }
      ...
      backendProcess.on('error', (error) => {
        ...
        if (process.platform === 'darwin') {
          guidance = 'On macOS, the first run of an unsigned binary may be blocked. ' +
